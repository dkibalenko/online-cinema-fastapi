...
class EmailSender(EmailSenderInterface):
    ...
    def comment_reply_template(self):
        """The name of the comment reply email template."""
    ...
    async def _send_email(
        ...
        try:
            ...
            await smtp.login(
                self._username,
                self._password,  # SMTP_USERNAME, SMTP_PASSWORD
            )
    ...
    async def send_activation_complete_email(
        ...
        await self._send_email(
            email, "Account Activated Successfully", html_content
        )
    ...
    async def send_password_reset_complete_email(
        ...
        await self._send_email(
            email, "Your Password Has Been Successfully Reset", html_content
        )

    async def send_custom_email(
        self, email: str, subject: str, template_name: str, context: dict
    ) -> None:
        ...
        """
        log.debug(f"Rendering custom template '{template_name}' for {email}")

        template = self._env.get_template(template_name)
        html_content = template.render(**context)

        log.debug(
            f"Rendered custom template ({len(html_content)} chars) for {email}"
        )
