...
from movies.models import Director, Genre, Star


async def get_or_create_related(
    model: type[Genre | Star | Director], names: list[str], db: AsyncSession
) -> list[Genre | Star | Director]:
    """Fetches related items by name from the DB or creates new ones if needed.
    ...
    # set comprehension avoids the unnecessary generator
    unique_names = list({name.strip() for name in names if name})
